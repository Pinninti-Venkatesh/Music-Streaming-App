...
const QueueItem = ({songCoverUrl,songName,songArtist,songDuration,songId,isPlaying}) => {
    ...
    return (
        ...
        <div className="col-span-1 text-gray-400 mr-3  active:text-gray-100"><Icon className="cursor-pointer hover:text-white" fontSize="large" onClick={()=>{playSong(songId)}} >{playingStatus ? 'pause' : 'play_arrow'}</Icon></div>
